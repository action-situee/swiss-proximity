    ...
    <v-row class="flex-grow-1" no-gutters>
      <v-col class="config-column" cols="3">
        ...
        <v-card-actions class="pt-4 px-2 px-xl-3 px-xxl-6">
          <v-btn variant="flat" @click="resetSessionStorage">
            Réinitialiser
          </v-btn>
        </v-card-actions>
      ...
      <v-col cols="9" class="map-column">
        <v-row class="fill-height" no-gutters>
          <v-col cols="6" class="relative pa-0">
            <VectorsMap
              class="left-map"
              :demand-variables="selectedDemandVariables"
              :supply-variables="selectedSupplyVariables"
              :list-tiles-params="listTilesParams"
              :selected-tiles-name="selectedTilesSource.demand"
              :colors="demandColors"
              :year="selectedYear"
              :distance="selectedDistance"
              :max-bounds="maxBoundsGeneva"
              :has-geocoder-search="false"
              :has-style-selector="true"
              @created:map="leftMap = $event"
            />
            <v-card-title class="overmap"> Demande </v-card-title>
          ...
          <v-col cols="6" class="relative pa-0">
            <VectorsMap
              ...
              :has-style-selector="false"
              @created:map="rightMap = $event"
            />
            <legend-map :colors="supplyColors" />
            <v-card-title class="overmap"> Offre </v-card-title>
          </v-col>
        </v-row>
      </v-col>
    </v-row>
...
import { maxBoundsGeneva } from "@/utils/map";
